  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <div className="relative flex flex-1 flex-col items-center bg-[#08090a] p-4 print:bg-white print:p-0">
        ...
        {quote.status === "accepted" && (
          <div className="mt-4 w-full">
            ...
            <div className="flex gap-2">
              ...
              {googleReviewLink && (
                <a
                  href={`https://wa.me/?text=${encodeURIComponent(`Hi ${quote.customer || "there"}, just wanted to say thanks for choosing us for ${quote.job}! If you're happy with the work, it would mean a lot if you could leave us a quick Google review — it really helps small businesses like ours. Here's the link: ${googleReviewLink}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-[#25d366]/30 bg-[#25d366]/10 py-3 font-barlow text-[12px] font-bold uppercase tracking-wide text-[#25d366]"
                >
              ...
              <button
                onClick={async () => {
                  if (reviewEmailSending) return;
                  setReviewEmailSending(true);
                  setReviewEmailResult(null);
                  const res = await fetch(`/api/quotes/${quote.id}/request-review`, { method: "POST" });
                  const d = await res.json();
                  setReviewEmailSending(false);
                  if (d.sent) setReviewEmailResult("sent");
                  else if (d.reason === "no_customer_email") setReviewEmailResult("no_email");
                  else setReviewEmailResult("error");
                  setTimeout(() => setReviewEmailResult(null), 4000);
                }}
                disabled={reviewEmailSending}
                className={`flex flex-1 items-center justify-center gap-1.5 rounded-xl border py-3 font-barlow text-[12px] font-bold uppercase tracking-wide transition-colors disabled:opacity-50 ${
                  reviewEmailResult === "sent"
                    ? "border-ok/30 bg-ok/10 text-ok"
                    : "border-line bg-panel text-textDim"
                }`}
              >
                ...
                {reviewEmailSending ? "Sending…" : reviewEmailResult === "sent" ? "Sent ✓" : "Email"}
              </button>
            </div>
            {reviewEmailResult === "no_email" && (
              <div className="mt-2 text-center text-[11px] text-textDim">No customer email on file — add one in the quote review.</div>
            )}
            {!googleReviewLink && (
              <div className="mt-2 text-center text-[10.5px] text-textDimmer">
        ...
        {/* ── Send payment link to customer (accepted quotes) ──────────── */}
        {quote.status === "accepted" && (
          <div className="mt-4 w-full">
            {bankDetails.paymentLink ? (
              <div className="rounded-2xl border border-line bg-panel p-4">
                <div className="mb-2 font-mono text-[10px] uppercase tracking-wider text-textDim">Send Payment Link</div>
                <div className="mb-3 flex items-center gap-2 rounded-xl border border-line bg-panelRaised px-3 py-2.5">
                  <span className="flex-1 truncate font-mono text-[10.5px] text-textDim">{bankDetails.paymentLink}</span>
                  <button
                    onClick={() => { navigator.clipboard.writeText(bankDetails.paymentLink); setStripePayLink("copied"); setTimeout(() => setStripePayLink(null), 2000); }}
                    className={`flex-none rounded-lg px-2.5 py-1 text-[10px] font-semibold transition-all ${stripePayLink === "copied" ? "bg-ok/15 text-ok" : "bg-line text-paper"}`}
                  >
                    {stripePayLink === "copied" ? "Copied!" : "Copy"}
                  </button>
                </div>
                <a
                  href={`https://wa.me/?text=${encodeURIComponent(`Hi ${quote.customer || "there"}, here is the payment link for your ${quote.job}: ${bankDetails.paymentLink}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#25d366] py-2.5 font-barlow text-[13px] font-bold uppercase tracking-wide text-white"
                >
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                  Send via WhatsApp
                </a>
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-line bg-panel px-4 py-3.5">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="flex-none text-textDimmer"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M2 10h20"/></svg>
                <div className="flex-1">
                  <div className="text-[12px] font-semibold">No payment link set up</div>
                  <div className="text-[11px] text-textDim">Add your Stripe, PayPal or GoCardless link in Settings so you can send it to customers.</div>
                </div>
                <button onClick={() => router.push("/settings")} className="flex-none text-[11px] font-bold text-hazard">Set up →</button>
              </div>
            )}
          </div>
        )}
