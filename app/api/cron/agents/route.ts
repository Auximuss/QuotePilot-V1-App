...
async function runScout(supabase: Supa) {
  ...
  // Try up to 4 city+trade combos to ensure we always find 10 fresh leads
  for (let attempt = 0; attempt < 4 && totalFound < 10; attempt++) {
    ...
    if (attempt === 0) {
      ...
      await agentLog(supabase, "Scout", `Only ${totalFound} so far — trying ${trade}s in ${city}…`, "info");
    }
    ...
    try {
      const gptRes = await openai.chat.completions.create({
        ...
        messages: [{
          role: "user",
          content: `List 25 real, independent UK ${trade} businesses based in ${city} or within 15 miles. Small local businesses (1–20 employees) only — no national chains or franchises. Return JSON: {"leads": [{"name": "Business Name", "domain": "domain.co.uk"}]}. Use .co.uk domains.`,
        }],
      });
      ...
      for (const candidate of candidates) {
        ...
        try {
          const check = await fetch(`https://${domain}`, { method: "HEAD", signal: AbortSignal.timeout(5000) });
          domainLive = check.status < 500;
        } catch {
          try {
            const check2 = await fetch(`https://www.${domain}`, { method: "HEAD", signal: AbortSignal.timeout(5000) });
      ...
      await agentLog(supabase, "Scout", `✗ Attempt ${attempt + 1} error: ${e.message}`, "error");
    }
  }
