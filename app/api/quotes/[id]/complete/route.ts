...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (quote.customer_email) {
    const paymentSection = biz?.payment_link
      ? `<p style="margin:16px 0"><a href="${h(biz.payment_link)}" style="background:#ff6a1f;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:bold">Pay Now</a></p>`
      : biz?.bank_account
      ? `<p style="margin:8px 0;color:#555">Bank transfer: <b>${h(biz.bank_name)}</b> | Sort: <b>${h(biz.bank_sort_code)}</b> | Acc: <b>${h(biz.bank_account)}</b></p>`
      : "";
    ...
    await sendEmail({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:520px;margin:0 auto;color:#1a1a1a">
          ...
          <p>Hi ${h(quote.customer_name) || "there"},</p>
          ...
          <table style="width:100%;border-collapse:collapse;margin:20px 0">
            <tr style="background:#f5f5f5">
              ...
              <td style="padding:8px;border:1px solid #ddd">${h(quote.job_title) || "—"}</td>
            </tr>
            <tr>
              <th style="padding:8px;text-align:left;border:1px solid #ddd">Address</th>
              <td style="padding:8px;border:1px solid #ddd">${h(quote.address) || "—"}</td>
            </tr>
            ${vat > 0 ? `
            <tr style="background:#f5f5f5">
            ...
            <tr>
              <th style="padding:8px;text-align:left;border:1px solid #ddd">VAT (20%) — ${h(biz?.vat_number)}</th>
          ...
          ${paymentSection}
          ${biz?.payment_terms ? `<p style="font-size:12px;color:#888;margin-top:16px">${h(biz.payment_terms)}</p>` : ""}
          <p style="margin-top:24px">Thank you for your business,<br><b>${h(biz?.name)}</b>${biz?.phone ? ` | ${h(biz.phone)}` : ""}</p>
        </div>
      `,
    });
  }
